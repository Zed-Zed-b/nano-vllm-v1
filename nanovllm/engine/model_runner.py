...
class ModelRunner:
    ...
    def warmup_model(self):
        ...
        for _ in range(num_seqs):
            seq = Sequence([0] * max_model_len)
            seqs.append(seq)
            num_scheduled_tokens[seq.seq_id] = max_model_len
        self.run_chunked_prefill(seqs, [], num_scheduled_tokens)
        torch.cuda.empty_cache()
    ...
    def prepare_chunked_prefill(self, all_seqs: list[Sequence], 
        ...
        if max_seqlen_q == 1:
            is_prefill = False # max_seqlen_q 为 1，说明每个 seq 只处理一个 token，走 decode 通道更快 
        else:
            is_prefill = True
            
        set_context(is_prefill = is_prefill, 
